...
class _EditorAutorunDefaults:
    ...
    max_symbols: int = 60
    recent_bars: int = 5
    rise_timeframe: str = "1m"
    rise_bars: int = 50
    rise_percent: float = 5.0
...
def _print_ar_report(symbol, timeframe, runtime, exchange, recent_alerts):
    ...
    print(f"\n===== {hdr} =====")
    print(
        f"عدد الشموع: {ln}  |  السعر الحالي: {_ar_num(last_close) if last_close is not None else '—'}  |  تغيّر 24 ساعة: {pct_s}"
    )
...
def __editor_autoargs__():
    ...
    if len(sys.argv) == 1:  # no flags -> inject defaults
        ...
        sys.argv += [
            ...
            "--rise-timeframe", defaults.rise_timeframe,
            "--rise-bars", str(defaults.rise_bars),
            "--rise-percent", f"{defaults.rise_percent}",
            "--min-change", "5",
            "--min-volume", "30000000",
