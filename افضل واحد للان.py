...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="الإطار الزمني المستخدم لاختبار نسبة الارتفاع قبل متابعة نتائج المسح",
    )
    parser.add_argument(
        "--rise-bars",
        type=int,
        default=0,
        help="عدد الشموع المعتمدة عند حساب نسبة الارتفاع المطلوبة",
    )
    parser.add_argument(
        "--rise-percent",
        type=float,
        default=0.0,
        help="أدنى نسبة ارتفاع (٪) يجب أن تتحقق قبل عرض مخرجات المؤشر",
    )
    parser.add_argument(
        "--pullback-report",
        action="store_true",
    ...
    if args.rise_percent > 0 and args.rise_bars <= 0:
        parser.error("--rise-bars يجب أن يكون رقمًا موجبًا عند تحديد --rise-percent")
...
def _android_cli_entry() -> int:
    ...
    for j, (sym, candles, rise_change) in enumerate(eligible, 1):
        ...
        if not recent_hits:
            ...
            print(
                f"[{j}/{eligible_total}] تخطي {_format_symbol(sym)} لعدم وجود أحداث خلال {span_phrase}"
            )
            continue
    ...
    if args.verbose:
        print(f"\\nتم. عدد الرموز: {eligible_total}  |  عدد التنبيهات: {alerts_total}")
    return 0
